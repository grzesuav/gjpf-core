 ...
 */
public class IntChoiceFromList extends IntChoiceGenerator {
  ...
  protected Integer parse (String varId, StackFrame resolveFrame){
    ...
    if (Character.isDigit(c)){ // its an integer literal
      ...
      if (o == null){
        throw new JPFException("no local or field '" + varId + "' value found for IntChoiceFromList " + id);
      }
      if (o instanceof Number){
        return new Integer( ((Number)o).intValue() * sign);
      } else {
        throw new JPFException("non-integer local or field '" + varId + "' value for IntChoiceFromList " + id);
      }
    }
  }
  ...
  public IntChoiceFromList randomize () {
