...
public abstract class ElementInfo
  ...
  private Memento<ElementInfo> cachedMemento;
  ...
  public String toString(){
    return ( ( classInfo != null ? classInfo.getName() : "ElementInfo" ) + '@' + Integer.toHexString(objectId) );
  }
  ...
  public void processReleaseActions(){
    ...
    classInfo.processReleaseActions(this);
  ...
  public boolean hasRefField(int objRef){
    return classInfo.hasRefField(objRef, fields);
  }
  ...
  public void hash(HashData hd){
    hd.add(classInfo.getClassLoaderInfo().getId());
    hd.add(classInfo.getId());
    fields.hash(hd);
    monitor.hash(hd);
  ...
  public void copyElements(ThreadInfo ti, ElementInfo eiSrc, int srcIdx, int dstIdx, int length){
    ...
    if (!isArray()) {
      throw new ArrayStoreException("destination object not an array: " + classInfo.getName());
    }
    if (!eiSrc.isArray()) {
    ...
    if (eiSrc.isReferenceArray() != isRefArray) {
      throw new ArrayStoreException("array types not compatible: " + eiSrc.getClassInfo().getName() + " and " + classInfo.getName());
    }
    ...
    if (isRefArray) {
      ClassInfo dstElementCi = classInfo.getComponentClassInfo();
  ...
  public String getArrayType(){
    ...
    return Types.getArrayElementType(classInfo.getType());
  }
  ...
  public boolean[] asBooleanArray(){
    if (fields instanceof ArrayFields) {
      ...
      throw new JPFException("not an array: " + classInfo.getName());
    }
  }
  ...
  public boolean isStringObject(){
    return ClassInfo.isStringClassInfo(classInfo);
  }
  ...
  public boolean instanceOf(String type){
    return Types.instanceOf(classInfo.getType(), type);
  }
  ...
  public void registerLockContender(ThreadInfo ti){
    ...
    assert ti.lockRef == MJIEnv.NULL || ti.lockRef == objectId :
  ...
  protected void checkAssertion(boolean cond, String failMsg){
    if (!cond) {
      ...
      if (tiLock != null) {
        System.out.println("locked by: " + tiLock);
      }
